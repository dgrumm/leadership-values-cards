...
- [ ] **Step 1**: Both More/Less Important piles have unlimited cards
- [ ] **Step 2**: Top 8 pile must have exactly 8 cards (no more, no less)
- [ ] **Step 3**: Top 3 pile must have exactly 3 cards (no more, no less)
- [ ] **All Steps**: Maximum 1 card in staging area at any time
- [ ] **All Steps**: Main deck must be empty before progression
- [ ] **All Steps**: Total cards in all drop zones must equal cards in deck at start of the step before progression (.e.g. all cards must be sorted into piles)

### Pile Limit Enforcement
- [ ] **Real-time Validation**: Check constraints on every card movement
- [ ] **Proactive Prevention**: Disable invalid drop zones when at limit
- [ ] **Bounce Animation**: Rejected cards return with 400ms elastic animation
- [ ] **Visual Indicators**: Pile borders change color when approaching/at limits
- [ ] **Counter Updates**: Live count display (e.g., "3/8" for Top 8 pile)

### Constraint Messaging
- [ ] **Warning Messages**: Clear, contextual feedback for violations
- [ ] **Auto-dismissal**: Messages disappear after 3-5 seconds
- [ ] **Progressive Disclosure**: Show hints before enforcing strict limits
- [ ] **Success Feedback**: Positive reinforcement when constraints satisfied

## Constraint Rules Engine
```javascript
const PILE_CONSTRAINTS = {
  step1: {
    more: { min: 1, max: Infinity },
    less: { min: 1, max: Infinity },
    staging: { max: 1 },
...
- [ ] **Pile States**:
  ...
  - Approaching limit: Orange border, warning glow
  - At limit: Red border, stop indicator
  - Valid drop target: Green border, success glow
...
## Acceptance Criteria
- [ ] All pile limits enforced consistently across steps
- [ ] Clear, helpful feedback provided for constraint violations
- [ ] Visual indicators accurately reflect constraint states
- [ ] Performance remains smooth during constraint checking
- [ ] Accessibility features work with constraint feedback
- [ ] Error recovery mechanisms function properly
- [ ] Progressive constraint introduction guides users effectively
