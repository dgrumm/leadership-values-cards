...
### Real-time Updates
- [ ] **Live Sync**: See card position changes when owner completes moves (debounced 200ms)
- [ ] **Smooth Transitions**: Cards animate to new positions for viewers
- [ ] **Update Indicators**: Highlight recently moved cards (300ms glow effect)
- [ ] **Activity Status**: Show when owner is actively rearranging

### Viewer Management
- [ ] **Join Notifications**: Announce when viewers enter/leave
- [ ] **Presence Tracking**: Track active viewers per reveal (max 10 viewers)
...
- [ ] **Read-only Interaction**: All drag/drop disabled for viewers

### Navigation Features
- [ ] **Quick Switch Menu**: Dropdown to switch between revealed participants
- [ ] **Back Navigation**: Return to participants list

### Performance Optimization
- [ ] **Update Batching**: Group card position updates (200ms debounce)
- [ ] **Memory Management**: Clean up viewer data on exit
- [ ] **Viewer Limits**: Maximum 10 viewers per arrangement
- [ ] **Network Efficiency**: Only sync on card drop completion, not during drag

## Live Update System
```javascript
// Simplified real-time arrangement updates
class ArrangementSync {
  constructor() {
    this.updateDebounceMs = 200;
    this.highlightDurationMs = 300;
  }
  ...
  handleArrangementUpdate(update) {
    ...
    cardPositions.forEach(cardUpdate => {
      this.animateCardTo(cardUpdate.cardId, cardUpdate.position);
    });
...
- 04.3 Reveal Mechanism ✓ (reveals must exist to view)
- Event-driven architecture (EventBus, viewer events) ✓
- Session-scoped stores for viewer state ✓
