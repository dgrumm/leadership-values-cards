...
import { setupStep3FastStart, resetAllTestStores, verifyStepState } from '../helpers/state-injection-helpers';

/**
 * FAST E2E tests for drag-and-drop functionality in Step 3 (Top 3 Selection)
 * Uses state injection to bypass slow Step 1+2 progression (12s → 0.5s setup)
 * Tests final selection constraints and completion flow
 */

test.describe('Step 3 - Drag and Drop Final Selection (Fast)', () => {
  let page: Page;

  test.beforeEach(async ({ page: testPage }) => {
    page = testPage;
    
    // Fast setup: Inject Steps 1+2 complete state and navigate to Step 3
    // This replaces 72 lines of slow manual progression
    await setupStep3FastStart(page, { sessionCode: 'FAST03' });
    
    // Verify state injection worked
    const stateValid = await verifyStepState(page, 3);
    if (!stateValid) {
      throw new Error('State injection failed - Step 3 not properly initialized');
    }
  });
  ...
  test('should enforce Top 3 pile limit (max 3 cards)', async () => {
    ...
    // Try to add a 4th card (should be rejected)
  ...
  test('should handle edge case of removing and re-adding cards', async () => {
    ...
    const stillHasCards = await page.locator('[data-testid="deck"], .deck-container').isVisible();
    if (stillHasCards) {
      await page.locator('[data-testid="deck"], .deck-container, button:has-text("Flip Next")').click();
      await page.waitForTimeout(200);
  ...
  test('performance: should complete full Step 3 workflow under time limit', async () => {
    const startTime = Date.now();
    
    // Complete a full Step 3 workflow: fill Top 3 pile
    for (let i = 0; i < 3; i++) {
      await page.locator('[data-testid="deck"], .deck-container, button:has-text("Flip Next")').click();
      const stagingCard = page.locator('[data-testid="staging-area"] .card, .staging-area .card').first();
      const top3Zone = page.locator('[data-pile="top3"], [data-testid="top3-pile"]');
      await stagingCard.dragTo(top3Zone);
      // Minimal waits for performance test
    }
    ...
    // Should complete Step 3 workflow in under 5 seconds (vs 15+ seconds in original)
    expect(duration).toBeLessThan(5000);
    console.log(`Step 3 workflow completed in ${duration}ms`);
  });
});